...
class Car(object):
    ...
    def __init__(self, position, velocity, world):
        ...
        self.state = [position, velocity]  #that is: [[y,x],[vy,vx]]
        self.world = world # [[0,0,0,...],[0,0,0,...]] world is a 2D list of values that range from 0-1
    ...
    def move(self, dt=1):
        ...
        # Predict the new position [y, x] based on velocity [vx, vy] and time, dt. i.e [newy, newx]
        predicted_position = [
        ...
        # Update the state
        self.state = [predicted_position, velocity]  #that is [[newy, newx],[vy, vx]]
        
        # Every time the robot moves, add the new position to the path
        self.path.append(predicted_position)  # [[y, x], [y , x],...]
    ...
    def turn_right(self):
        """ Turning right "rotates" the velocity values, so vy = +vx, and vx = -vy.
            ...
            For example, if a car is going right at 1 world cell/sec this means 
            vy = 0, vx = 1, 
            and if it turns right again, then it should be moving downwards on the world grid 
            at the same speed! 
            And up is vy = 1 and vx = 0
            """
